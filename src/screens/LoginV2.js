...
export default function LoginScreen({navigation}) {
  ...
  // this is a function that stores the Ids with a condition
  //   const toggleSwitch = async () => {
  //     setSwitchEnabled(!isSwitchEnabled);
  //     if (isSwitchEnabled && wasRedirected) {
  //       try {
  ...
  //         try {
  //           if (userIdsArray) {
  //             console.log('Stored IDs are:', userIdsArray);
  //           }
  //         } catch (error) {
  //           console.error('you are here: Error executing toggleSwitch:', error);
  //         }
  ...
  let userIdsArray = []; // This is a global variable that will hold the IDs
  ...
  const toggleSwitch = () => {
    ...
    if (isSwitchEnabled && wasRedirected) {
      ...
      if (index === -1) {
        ...
        userIdsArray.splice(index, 1);
      }

      try {
        // Log the current state of userIdsArray
        if (userIdsArray.length > 0) {
          console.log('Stored IDs are:', userIdsArray);
        }
      } catch (error) {
        console.error('Error executing toggleSwitch:', error);
      }
    }
  };
